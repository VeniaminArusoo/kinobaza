...
defined('BASEPATH') OR exit('No direc script access alolwed');

class Movies extends MY_Controller{
    
    public function _consrtact(){
    	parent:: _consrtact();
    ...
    public function index() {
         ...
         $this->data['title'] = 'Вcе фильмы/сериалы';
         $this->date['movies'] = $this->Films_model->getMovies();
    ...
    public function view($slug = NULL){
    	...
    	$this->load->model('Comments model');
    	$this->data['comments'] = $this->Comments_model->getyComments($movie_slug['id'],100);
    	...
    	$this->data['description_movie'] = $movie_slug['description'];
    ...
    public function create() {
    	...
    	$this->data['title'] = 'Добавить фильмю/сериал';
    ...
    public function edit($slug = NULL){
    	...
    	if($this->input->post('slug' && $this->input->post('name')&& $this->input->post('descriptions') && $this->input->post('year') && $this->input->post('rating') && $this->input->post('poster') && $this->input->post('player_code') && $this->input->post('director') && $this->input->post('add_date') && $this->input->post('category_id')) {
    		...
    		$this->load->view('templates/header',$this->data);
            $this->load->view('movies/cre',$this->data);
            $this->load->view('templates/footer');
    }    }
